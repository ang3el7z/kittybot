    ...
    public function action()
    {
        switch (true) {
            ...
            case preg_match('~^/ports$~', $this->input['callback'], $m):
                $this->ports();
                break;
            case preg_match('~^/services$~', $this->input['callback'] ?: $this->input['message'], $m):
                $this->servicesMenu();
                break;
            case preg_match('~^/service (?P<action>start|stop|restart|enable|disable) (?P<service>\w+)$~', $this->input['callback'], $m):
                $this->serviceAction($m['service'], $m['action']);
                break;
    ...
    public function checkVersion()
    {
        try {
            ...
            if (!empty($c['admin']) && (empty($this->time) || ((time() - $this->time) > 3600))) {
                ...
                $last       = file_get_contents("https://raw.githubusercontent.com/ang3el7z/kittybot/$b/version");
                        ...
                        foreach ($c['admin'] as $k => $v) {
                            $this->send($v, implode("\n", $diff), 0, [
                                [
                                    [
                                        'text'    => 'changelog',
                                        'web_app' => ['url' => "https://raw.githubusercontent.com/ang3el7z/kittybot/$b/version"],
                                    ],
                                    [
...
<code>example.org https://dns.adguard.com/dns-query 94.140.14.14</code>",
    ...
    private function containers(): ContainerService
    {
    ...
    public function servicesMenu()
    {
        try {
            ...
            $this->update($this->input['chat'], $this->input['message_id'], 'Services error: ' . $e->getMessage(), [
                [[
                    'text'          => $this->i18n('back'),
                    'callback_data' => "/menu config",
                ]],
        ...
        $text[] = '';
        $text[] = 'Core services cannot be disabled.';

        foreach (ServiceCatalog::CORE as $service) {
            $data[] = [[
                'text'          => "core $service",
                'callback_data' => "/service restart $service",
            ]];
        }

        foreach (ServiceCatalog::OPTIONAL as $service) {
            $enabled = $states[$service] ?? true;
            $data[] = [
                [
                    'text'          => ($enabled ? 'on ' : 'off ') . $service,
                    'callback_data' => '/service ' . ($enabled ? 'disable' : 'enable') . " $service",
                ],
                [
                    'text'          => 'restart',
                    'callback_data' => "/service restart $service",
